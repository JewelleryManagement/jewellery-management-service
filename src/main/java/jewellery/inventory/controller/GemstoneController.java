...
public class GemstoneController {
  ...
  @PostMapping
  public ResponseEntity<GemstoneDTO> createGemstone(
      @RequestBody @Valid GemstoneDTO gemstoneDTO, UriComponentsBuilder uriComponentsBuilder) {
    URI location =
        uriComponentsBuilder
            .path("/api/v1/gemstone/{id}")
  ...
  @PutMapping("/{id}")
  public ResponseEntity<GemstoneDTO> updateGemstone(
      @PathVariable("id") Long id, @Valid @RequestBody GemstoneDTO gemstoneDTO) {
  ...
  public ResponseEntity<String> deleteGemstoneById(@PathVariable("id") Long id) {
    ...
    return new ResponseEntity<>(
        "Gemstone with id: " + id + " has been deleted successfully!!", HttpStatus.OK);
  }
}
